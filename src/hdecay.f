...
C     to give in the masses of the particles as follows:
C     am1cx = lightest visible particle
C     am2cx = next-to-lightest visible particle
C     am3cx = DM particle
C ==================================================================      
...
c MMM changed 28/4/15
      ...
      READ(NI,*)
      READ(NI,*)
      READ(NI,101)ISINGLET
      READ(NI,101)ICXSM
      READ(NI,*)
      READ(NI,100)SBALPHACX1
      READ(NI,100)SBAMH1
      READ(NI,100)SBAMH2
      READ(NI,100)SBVS
      READ(NI,*)
      ...
      READ(NI,100)SDAMHD
      READ(NI,100)SDAMH2S
      READ(NI,100)SDLAMBDAS
      READ(NI,*)
      READ(NI,100)BALPHACX1
      READ(NI,100)BALPHACX2
      READ(NI,100)BALPHACX3
      READ(NI,100)BAM1CX
      READ(NI,100)BAM3CX
      READ(NI,100)BVSCX
      READ(NI,*)
      READ(NI,100)DALPHACX1
      READ(NI,100)DAM1CX
      READ(NI,100)DAM2CX
      READ(NI,100)DAM3CX
      READ(NI,100)DVSCX
...
c calculate the EW corrections and choose the renormalization scheme      
      ...
      READ(NI,101) ipdprocess
      READ(NI,101) iralph_mix
      READ(NI,*)
      READ(NI,100) DeltaE
      READ(NI,*)
...
c-----if a1!=0 do not calculate ELW corrections
      ...
      if(ielwcxsm.eq.1) then
         if(da1cx.ne.0.D0) then
             ...
             write(*,*)"ielwcxsm is set to 0."
             write(*,*)"#####"
             ielwcxsm=0
             NNLOapprox=0
            endif
      endif
c-----Call CxSMEWCorrections also at LO because it also does the parameter conversion
c----- so that the parameters will be converted from one scheme into another
...
20    FORMAT(A74)
     ...
     .          NSUSYAA=69,NSUSYAB=68,NSUSYAC=67,NSUSYAD=66,NSUSYAE=65,
     .          NSUSYCA=64,NSUSYCB=63,NSUSYCC=62,NSUSYCD=61,NSUSYCE=60,
     .          NCX1=51,NCX2=52,NCX3=53,NCX4=54,NCX5=55,NCX6=56,
     .          NCX7=57,NCX8=58,NCX9=59)
...
c end MMM changed 8/12/15
      ...
      IF(IHIGGS.EQ.0.and.icxsm.eq.0) THEN
...
c MMM changed 8/12/2015
      if(icxsm.eq.1.or.icxsm.eq.2.or.icxsm.eq.3.or.icxsm.eq.4) then
      WRITE(NCX1,70)'MH1   ','BB   ','TAU TAU','MU MU ','SS ','CC ','TT'
      WRITE(NCX1,69)
      WRITE(NCX1,*)
      WRITE(NCX2,70)'MH1   ','GG ','GAM GAM','Z GAM ','WW ','ZZ ','WIDTH
     .'
      WRITE(NCX2,69)
      WRITE(NCX2,*)
      endif
      if(icxsm.eq.3) then
      WRITE(NCX3,70)'MH1   ','H2 H2','H2 H3','H3 H3 '
      WRITE(NCX3,69)
      WRITE(NCX3,*)
      endif
      if(icxsm.eq.4) then
      WRITE(NCX3,70)'MH1   ','H2 H2','H2 HD','HD HD '
      WRITE(NCX3,69)
      WRITE(NCX3,*)
      endif
      if(icxsm.eq.1) then
      WRITE(NCX3,70)'MH1   ','H2 H2'
      WRITE(NCX3,69)
      WRITE(NCX3,*)
      endif
      if(icxsm.eq.2) then
      WRITE(NCX3,70)'MH1   ','HD HD'
      WRITE(NCX3,69)
      WRITE(NCX3,*)
      endif

      if(icxsm.eq.1.or.icxsm.eq.3.or.icxsm.eq.4) then      
      WRITE(NCX4,70)'MH2   ','BB   ','TAU TAU','MU MU ','SS ','CC ','TT'
      WRITE(NCX4,69)
      WRITE(NCX4,*)
      WRITE(NCX5,70)'MH2   ','GG ','GAM GAM','Z GAM ','WW ','ZZ ','WIDTH
     .'
      WRITE(NCX5,69)
      WRITE(NCX5,*)
      endif
      if(icxsm.eq.3) then
      WRITE(NCX6,70)'MH2   ','H1 H1','H1 H3','H3 H3 '
      WRITE(NCX6,69)
      WRITE(NCX6,*)
      endif
      if(icxsm.eq.4) then
      WRITE(NCX6,70)'MH2   ','H1 H1','H1 HD','HD HD '
      WRITE(NCX6,69)
      WRITE(NCX6,*)
      endif
      if(icxsm.eq.1) then
      WRITE(NCX6,70)'MH2   ','H1 H1'
      WRITE(NCX6,69)
      WRITE(NCX6,*)
      endif

      if(icxsm.eq.3) then
      WRITE(NCX7,70)'MH3   ','BB   ','TAU TAU','MU MU ','SS ','CC ','TT'
      WRITE(NCX7,69)
      WRITE(NCX7,*)
      WRITE(NCX8,70)'MH3   ','GG ','GAM GAM','Z GAM ','WW ','ZZ ','WIDTH
     .'
      WRITE(NCX8,69)
      WRITE(NCX8,*)
      WRITE(NCX9,70)'MH3   ','H1 H1','H1 H2','H2 H2 '
      WRITE(NCX9,69)
      WRITE(NCX9,*)
      endif
c end MMM changed 8/12/2015
...
c MMM changed 8/12/15
      DIMENSION RESCAL(3)
      DIMENSION WTOTCX(3),SCXWDTH(3),SCXBRT(3),SCXBRB(3),SCXBRL(3),
     .     SCXBRM(3),SCXBRC(3),SCXBRS(3),SCXBRG(3),SCXBRGA(3),
     .     SCXBRZGA(3),SCXBRW(3),SCXBRZ(3)
c end MMM changed 8/12/15      
      double precision mbmsbar,mbl,mbu
      character spinfo1*100,spinfo2*100,modselval*100,mincom(1:20)*20,
     .          extcom(0:100)*20,softcom(1:100)*20,hmixcom(1:10)*20,
...
c MMM changed 8/12/2015
      if(icxsm.eq.1.or.icxsm.eq.2.or.icxsm.eq.3.or.icxsm.eq.4)then
      write(ncx1,20)am1cx,scxbrb(1),scxbrl(1),scxbrm(1),scxbrs(1),
     .        scxbrc(1),scxbrt(1)
      write(ncx2,20)am1cx,scxbrg(1),scxbrga(1),scxbrzga(1),scxbrw(1),
     .     scxbrz(1),scxwdth(1)
      endif

      if(icxsm.eq.3.or.icxsm.eq.4) then
      write(ncx3,20)am1cx,scxbrh1h2h2,scxbrh1h2h3,scxbrh1h3h3
      endif
      if(icxsm.eq.1.or.icxsm.eq.2) then
      write(ncx3,20)am1cx,scxbrh1h2h2
      endif

      if(icxsm.eq.1.or.icxsm.eq.3.or.icxsm.eq.4)then
      write(ncx4,20)am2cx,scxbrb(2),scxbrl(2),scxbrm(2),scxbrs(2),
     .        scxbrc(2),scxbrt(2)
      write(ncx5,20)am2cx,scxbrg(2),scxbrga(2),scxbrzga(2),scxbrw(2),
     .     scxbrz(2),scxwdth(2)
      endif

      if(icxsm.eq.3.or.icxsm.eq.4)then
      write(ncx6,20)am2cx,scxbrh2h1h1,scxbrh2h1h3,scxbrh2h3h3
      endif

      if(icxsm.eq.1)then
      write(ncx6,20)am2cx,scxbrh2h1h1
      endif

      if(icxsm.eq.3) then
      write(ncx7,20)am3cx,scxbrb(3),scxbrl(3),scxbrm(3),scxbrs(3),
     .        scxbrc(3),scxbrt(3)
      write(ncx8,20)am3cx,scxbrg(3),scxbrga(3),scxbrzga(3),scxbrw(3),
     .     scxbrz(3),scxwdth(3)
      write(ncx9,20)am3cx,scxbrh3h1h1,scxbrh3h1h2,scxbrh3h2h2
      endif
c end MMM changed  8/12/2015
...
c MMM changed 28/4/15
      DIMENSION RESCAL(3)
      DIMENSION HGGCX(3),DCCCX(3),DBBCX(3),HMMCX(3),HLLCX(3),HSSCX(3),
     .     HCCCX(3),HBBCX(3),HTTCX(3),HGACX(3),HZGACX(3),HWWCX(3),
     .     HZZCX(3),WTOTCX(3),SCXWDTH(3),SCXBRT(3),SCXBRB(3),SCXBRL(3),
     .     SCXBRM(3),SCXBRC(3),SCXBRS(3),SCXBRG(3),SCXBRGA(3),
     .     SCXBRZGA(3),SCXBRW(3),SCXBRZ(3)
c end MMM changed 28/4/15      
...
c MMM changed 8/12/15
C =========================================================
C                    SM or SINGLET HIGGS DECAYS
C =========================================================

         if(isinglet.eq.1) then
            if(icxsm.eq.2) then
               ii = 1
            elseif(icxsm.eq.1.or.icxsm.eq.4) then
               ii = 2
            elseif(icxsm.eq.3) then
               ii = 3
            endif
         elseif(isinglet.eq.0) then
            ii = 1
            rescal(1) = 1.D0
            rescal(2) = 1.D0
            rescal(3) = 1.D0
...
c            print*,'vals',amsm,amh,rescal(i),i
c end MMM changed 8/12/15
...
c MMM changed 26/2022       
       if(ielwcxsm.eq.1) then
          HLL=HFF(AMH,(AMTAU/AMH)**2)*(1.D0+dcxsmll)        
       endif
...
c MMM changed 26/2022       
       if(ielwcxsm.eq.1) then
          HB1=3.D0*HFF(AMH,(AMB/AMH)**2)*
     .    TQCDH(AMB**2/AMH**2)*(1.D0+dcxsmbb)
     .   + DBB          
       endif
c end MMM changed 26/2022 
      ENDIF
...
c MMM changed 26/8/22         
         if(ielwcxsm.eq.1) then
            HT2=3.D0*HFF(AMH,(RMT/AMH)**2) *
     .      QCDH(RMT**2/AMH**2,5)*(1.D0+dcxsmtt)
         endif
c end MMM changed 26/8/22          
        ENDIF
        IF(HT2.LT.0.D0) HT2 = 0
...
c MMM changed 26/8/22         
         if(ielwcxsm.eq.1) then
            HT1=3.D0*HFF(AMH,(AMT/AMH)**2) * 
     .      TQCDH(AMT**2/AMH**2)*(1.D0+dcxsmtt)
         endif
c end MMM changed 26/8/22          
        ENDIF
        RAT = 2*AMT/AMH
...
c The Higgs-to-Higgs decays
      ...
      if(i.eq.1) then
         if(am1cx.ge.2.D0*am2cx) then
            h1h2h2=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM2cx**2/AMH**2)*gg122**2
         else
            h1h2h2=0.D0
         endif
         if(am1cx.ge.2.D0*am3cx) then
            h1h3h3=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM3cx**2/AMH**2)*gg133**2
c MMM changed 26/8/22            
            if(ielwcxsm.eq.1) then
               h1h3h3=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .              BETA_HDEC(AM3cx**2/AMH**2)*gg133**2*
...
c end MMM changed 26/8/22             
         ...
         sumfinmass=am2cx+am3cx
         if(am1cx.ge.sumfinmass) then
            h1h2h3=GF/8D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           lamb_hdec(AM2cx**2/AMH**2,AM3cx**2/AMH**2)*gg123**2
         else
            h1h2h3=0.D0
         endif
      endif
      ...
      if(i.eq.2) then
         if(am2cx.ge.2.D0*am1cx) then
            h2h1h1=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM1cx**2/AMH**2)*gg112**2
c MMM changed 26/8/22            
            if(ielwcxsm.eq.1) then
               h2h1h1=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .              BETA_HDEC(AM1cx**2/AMH**2)*gg112**2*
...
c end MMM changed 26/8/22  
         ...
         if(am2cx.ge.2.D0*am3cx) then
            h2h3h3=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM3cx**2/AMH**2)*gg233**2
c MMM changed 26/8/22            
            if(ielwcxsm.eq.1) then
               h2h3h3=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM3cx**2/AMH**2)*gg233**2*
...
c end MMM changed 26/8/22              
         ...
         sumfinmass=am1cx+am3cx
         if(am2cx.ge.sumfinmass) then
            h2h1h3=GF/8D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           lamb_hdec(AM1cx**2/AMH**2,AM3cx**2/AMH**2)*gg123**2
         else
            h2h1h3=0.D0
         endif
      endif
      ...
      if(i.eq.3) then 
         if(am3cx.ge.2.D0*am1cx) then
            h3h1h1=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM1cx**2/AMH**2)*gg113**2
         else
            h3h1h1=0.D0
         endif
         if(am3cx.ge.2.D0*am2cx) then
            h3h2h2=GF/16D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           BETA_HDEC(AM2cx**2/AMH**2)*gg223**2
         else
            h3h2h2=0.D0
         endif
         sumfinmass=am1cx+am2cx
         if(am3cx.ge.sumfinmass) then
            h3h1h2=GF/8D0/DSQRT(2D0)/PI*AMZ**4/AMH*
     .           lamb_hdec(AM1cx**2/AMH**2,AM2cx**2/AMH**2)*gg123**2
         else
            h3h1h2=0.D0
         endif
      endif
...
c      if(i.eq.1) then
c         print*,'h1h2h2',h1h2h2
c         print*,'h1h3h3',h1h3h3
c         print*,'h1h2h3',h1h2h3
c      elseif(i.eq.2) then
c         print*,'h2h1h1',h2h1h1
c         print*,'h2h3h3',h2h3h3
c         print*,'h2h1h3',h2h1h3
c      elseif(i.eq.3) then
c         print*,'h3h1h1',h3h1h1
c         print*,'h3h2h2',h3h2h2
c         print*,'h3h1h2',h3h1h2
c      endif
      endif
c end MMM changed 8/12/15      
...
c MMM changed 8/12/15
      if(isinglet.eq.1) then
         wtotcx(i) = 0.D0
         WTOTCX(i)=(HLL+HMM+HSS+HCC+HBB+HTT+HGG+HGA+HZGA+HWW+HZZ)*
     .        rescal(i)**2
c         print*,'vals2',wtotcx(i),rescal(i),i
         if(i.eq.1) then
            wtotcx(i)=wtotcx(i)+h1h2h2+h1h3h3+h1h2h3
         endif
         if(i.eq.2) then
            wtotcx(i)=wtotcx(i)+h2h1h1+h2h3h3+h2h1h3
         endif
         if(i.eq.3) then
            wtotcx(i)=wtotcx(i)+h3h1h1+h3h2h2+h3h1h2
         endif
         IF(WTOTCX(i).NE.0.D0)THEN
            SCXBRT(i)=HTT*rescal(i)**2/WTOTCX(i)
            SCXBRB(i)=HBB*rescal(i)**2/WTOTCX(i)
            SCXBRL(i)=HLL*rescal(i)**2/WTOTCX(i)
            SCXBRM(i)=HMM*rescal(i)**2/WTOTCX(i)
            SCXBRC(i)=HCC*rescal(i)**2/WTOTCX(i)
            SCXBRS(i)=HSS*rescal(i)**2/WTOTCX(i)
            SCXBRG(i)=HGG*rescal(i)**2/WTOTCX(i)
            SCXBRGA(i)=HGA*rescal(i)**2/WTOTCX(i)
            SCXBRZGA(i)=HZGA*rescal(i)**2/WTOTCX(i)
            SCXBRW(i)=HWW*rescal(i)**2/WTOTCX(i)
            SCXBRZ(i)=HZZ*rescal(i)**2/WTOTCX(i)
            if(i.eq.1) then
               SCXBRH1H2H2=h1h2h2/wtotcx(i)
               SCXBRH1H3H3=h1h3h3/wtotcx(i)
               SCXBRH1H2H3=h1h2h3/wtotcx(i)
            elseif(i.eq.2) then
               SCXBRH2H1H1=h2h1h1/wtotcx(i)
               SCXBRH2H3H3=h2h3h3/wtotcx(i)
               SCXBRH2H1H3=h2h1h3/wtotcx(i)
            elseif(i.eq.3) then
               SCXBRH3H1H1=h3h1h1/wtotcx(i)
               SCXBRH3H2H2=h3h2h2/wtotcx(i)
               SCXBRH3H1H2=h3h1h2/wtotcx(i)
            endif
         ELSE
            SCXBRT(i)=0
            SCXBRB(i)=0
         ...
         ENDIF
         SCXWDTH(i)=WTOTCX(i)
c         print*,'vals3',scxwdth(i),SCXBRB(i),SCXBRL(i)
      endif
c end MMM change 8/12/15
